...
func (a *KNN[T]) Search(ctx context.Context, lat float64, long float64, filter FilterFunc[T]) []T {
	a.pruneMutex.RLock()
	defer a.pruneMutex.RUnlock()
	...
	for {
		...
		if poppedNode.IsLeaveNode() {
			...
			stopped := false
			results, stopped = poppedNode.FilerValues(ctx, results, distance, filter)
			if stopped {
				return results
			}
		} else {
